            ...
            if (name === "partBegin") {
                ...
                switch (part.transferEncoding) {
                    ...
                    case "utf-8": {
                        const dataPropagation = (ctx) => {
                            if (ctx.name === "partData") {
                                part.emit(
                                    "data",
                                    ctx.buffer.slice(ctx.start, ctx.end)
                                );
                            }
                        };
                  ...
                  */
                                const offset =
                                    parseInt(
                                        part.transferBuffer.length / 4,
                                        10
                                    ) * 4;
                                part.emit(
                                    "data",
                                    Buffer.from(
                                        part.transferBuffer.substring(
                                            0,
                                            offset
                                        ),
                                        "base64"
                                ...
                                part.transferBuffer =
                                    part.transferBuffer.substring(offset);
                            }
                        };
                        const dataStopPropagation = (ctx) => {
                            if (ctx.name === "partEnd") {
                                part.emit(
                                    "data",
                                    Buffer.from(part.transferBuffer, "base64")
                                );
                                part.emit("end");
                                parser.off("data", dataPropagation);
                                parser.off("data", dataStopPropagation);
...
const createNecessaryDirectoriesAsync = decorateForceSequential(function (
    filePath
) {
...
class IncomingForm extends node_events.EventEmitter {
    ...
    async _handlePart(part) {
        if (
            ...
            typeof part.originalFilename !== "string"
        ) {
            this._error(
                new FormidableError(
    ...
    _getExtension(str) {
        ...
        const firstInvalidIndex =
            Array.from(rawExtname).findIndex(invalidExtensionChar);
